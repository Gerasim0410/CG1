...
namespace CG1
{
    ...
    class Opening : Morphology // Морфологическое открытие (размыкание)
    {
    ...
    class Closing : Morphology // Морфологическое закрытие (замыкание)
    {
        ...
        public override Bitmap processImage(Bitmap sourceImage, BackgroundWorker worker)
        {
            ...
            for (int i = 0; i < sourceImage.Width; i++)
            {
                ...
                for (int j = 0; j < sourceImage.Height; j++)
                {
                    ...
                    int newR = Clamp(Math.Abs(sourceImage.GetPixel(i, j).R - resultcL.GetPixel(i, j).R), 0, 255);
                    int newG = Clamp(Math.Abs(sourceImage.GetPixel(i, j).G - resultcL.GetPixel(i, j).G), 0, 255);
                    int newB = Clamp(Math.Abs(sourceImage.GetPixel(i, j).B - resultcL.GetPixel(i, j).B), 0, 255);
                    resultImage.SetPixel(i, j, Color.FromArgb(newR, newG, newB));
                }
            }
            return resultImage;
